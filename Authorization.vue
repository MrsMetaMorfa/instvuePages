...
.login_form
  ...
  h1.p_title Авторизация
  ErrorsBlock(:errors="errors")
  form.form-login(action="/auth/login", method="POST")
    input.hide(type="hidden" name="_token" :value="this.$store.state.csrf_token")
    label.form-login_line
      span.u-visually-hidden Укажите свой email
      input(type="email", name="email", id="loginEmail", placeholder="Укажите свой email", @focus="setActiveLine")
    label.form-login_line
      input(type="password", name="password", id="loginPassword", placeholder="Укажите ваш пароль", @focus="setActiveLine")
    ...
    button.btn.btn-default(type="submit" id="loginButton" @click.prevent="sendAuthForm") Войти
  .buttons-container.container.container--justifyed
    a.btn.link.create-account(type='button', href='/signup') Создать аккаунт
    button.btn.link.forgot-password(type='button' @click="$emit('forgotClicked')") Забыли пароль?
  a.btn.btn-default.login-instagram(href="/auth/instagram")
    img(src="../../images/instagram.png", alt="Instagram Logo")
    | Войти через Instagram

  a.link.tos(href="/privacy", target="_blank") Политика конфиденциальности
...
export default {
  ...
  methods: {
    ...
    toggleRegistration: function () {
      this.$emit('toggleRegistration', true)
    },
    sendAuthForm: function ($event) {
        let self = this;
        let button = $event.target;
        ...
        if (!login_data.email.length) {
            return self.showErrors([
                "Введите ваш email"
            ]);
        }

        if (!login_data.password.length) {
            return self.showErrors([
                "Введите ваш пароль"
            ]);
        }

        if (login_data.password.length > 32 || login_data.password.length < 6) {
            return self.showErrors([
        ...
        let match = login_data.password.match(this.$store.state.password_regex);

        if (!match || match[0].length !== login_data.password.length) {
            return self.showErrors([
                "Пароль должен содержать только буквы латинского алфавита и цифры"
            ]);
        }

        match = login_data.email.match(this.$store.state.email_regex);

        if (!match || match[0].length !== login_data.email.length) {
            return self.showErrors([
        ...
        .then(response => {
            window.location = "/"; // авторизовались
        })
        .catch(res => {
